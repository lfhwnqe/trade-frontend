...
export function ImageUploader({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log('开始上传到S3:', { uploadUrl, fileType: file.type, fileSize: file.size });
      // 直传S3
    ...
    <div className="mb-3">
      ...
      <div className="flex gap-3 flex-wrap items-center">
        {value.map((img, idx) => (
          <div
            key={img.key}
            className="relative w-28 h-28 border rounded bg-muted flex items-center justify-center group"
          >
            {img.url ? (
              <img
            ...
            {!disabled && (
              <button
                type="button"
                className="absolute -top-2 -right-2 bg-white rounded-full shadow p-0.5 invisible group-hover:visible hover:bg-red-500 hover:text-white transition"
                style={{ zIndex: 2 }}
        ...
        {!reachMax && (
          <div
            className="w-28 h-28 border flex flex-col items-center justify-center rounded bg-muted hover:bg-gray-100 transition cursor-pointer"
